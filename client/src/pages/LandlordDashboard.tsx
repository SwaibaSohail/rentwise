  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) setEditing(null); }}>
            <DialogTrigger asChild>
              <Button onClick={() => setEditing(null)} className="font-semibold">Add property</Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader>
          ...
          <section className="mb-10">
            ...
            <ul className="grid gap-3">
              {pendingApplications.map((app) => (
                <li key={app.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border/60 bg-card p-4 shadow-sm">
                  <div>
        ...
        {/* Maintenance tickets */}
        <section className="mb-10">
          <h2 className="mb-4 text-lg font-semibold">Maintenance tickets</h2>
          ...
          {conversations.length === 0 ? (
            ...
            <ul className="grid gap-3">
              {conversations.map((conv) => (
                <li key={conv.id} className="flex items-center justify-between rounded-xl border border-border/60 bg-card p-4 shadow-sm">
                  <div>
        ...
        {/* Payments */}
        <section className="mb-10">
          <h2 className="mb-4 text-lg font-semibold">Payments received</h2>
          {receivedPayments.length === 0 ? (
            <p className="text-sm text-muted-foreground">No payments received yet.</p>
          ) : (
            <ul className="grid gap-3">
              {receivedPayments.map((p) => (
                <li key={p.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border/60 bg-card p-4 text-sm shadow-sm">
                  <div>
                    <p className="font-medium">{p.tenant?.name ?? "Tenant"}</p>
                    <p className="text-muted-foreground">{p.tenancy?.property?.title ?? "Property"}</p>
                  </div>
                  <span className="font-semibold">${(p.amount / 100).toLocaleString()}</span>
                  <span
